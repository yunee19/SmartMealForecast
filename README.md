...
* XGBoost는 강력한 부스팅 알고리즘으로 구현되었으며, **점심과 저녁 모두에서 가장 낮은 MAE = 2.52%**를 기록했습니다. 이는 RF 대비 거의 절반 수준으로 MAE가 감소한 것이며, XGB가 데이터의 특성을 더 효율적으로 활용하여 예측 정확도를 크게 향상시킴을 보여줍니다. 실제로 XGBoost는 낮은 MAE와 정규화 MAE 수치를 모두 기록하며, 이 프로젝트에서 핵심 모델로 자리잡았습니다.
